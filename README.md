...
- This repository is divided into 5 branches:
1. **main** - contains the main `README.md` with detailed info. Other branches are derived from main. 
2. **jenkins_job** - branch create different types of jenkins pipelines and syntax of Jenkinsfile
3. **feature/payment** - branch created to test multibranch pipeline
4. **jenkins-shared-lib** - branch to test integration of shared jenkins library 
5. **increment_version** - branch to test automatic incrementation of image version. 
...
- Freestyle job is executing a single task.
...
## Push docker image to private DockerHub repository in Jenkins Build
- tested in **jenkins_job** branch
- docker hub private repository has to be created.
- docker hub private repository credentials must be saved in Jenkins.
- Jenkins job - Configuration - Build Environment - use secret text binding : USERNAME and PASSWORD variable names are defined here and already existing credentials are linked to those variables. They can be referenced in the shell command and used for *docker build* and *docker push* commands.

## Push docker image to Nexus Repository
- insecure registry of Nexus must be specified inside `/etc/docker/daemon.json` file on Jenkins server
- docker container has to be restarted to load insecure registry
...
- execute specific pipeline for multiple branches **main, jenkins_job, feature/payment**
- *when* expression may help to decide for which branch the specific stage to be executed. Environmental variable BRANCH_NAME is available in multibranch build and holds the information about current branch name. 
...
- tested in the branch **jenkins-shared-libs**
...
- Shared library can be set globally in the Jenkins setup (Manage Jenkins - System - Global Pipeline Libraries) or as project scoped library. Library has to be connected in the Jenkinsfile:
    *library identifier: 'jenkins-shared-library@main', retriever: modernSCM(* <br>
    *[$class: 'GitSCMSource',* <br>
    *remote: 'URL to Git Repo',* <br>
    *credentialsId: 'github-credentials'])* <br>

## Dynamically Increment Version 
- application version has 3 parts: **major_version.minor_version.patch**
- for Maven application we can use the plugin that parses the version into 3 parts and call the plugin as a shell command. Example updating patch version: <br>
